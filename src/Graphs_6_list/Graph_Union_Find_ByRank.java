...
public class Graph_Union_Find_ByRank {
	...
	static class Node {
		...
		static class UnionFind {
			...
			public void Union(int u, int v) {
				...
				if (Ul_u == Ul_v)
					return;
				// case 1 if u's rank is less than v
				if (rank[Ul_u] < rank[Ul_v]) {
					parent[Ul_u] = Ul_v;
				} else if (rank[Ul_v] < rank[Ul_u]) { // case 2 :- if v's rank is greater than u
					parent[Ul_v] = Ul_u;
				} else { // if Ranks of u and v are Equal choose one
					parent[Ul_v] = Ul_u;
					rank[Ul_u]++;
				}
		...
		public static void main(String[] args) {
			...
			for (int i = 0; i < numofQ; i++) {
				...
				if (uf.isConnected(u, v))
					System.out.println(srcData + " and " + dstData + " are connected.");
				else
					System.out.println(srcData + " and " + dstData + " are Not connected.");
			}
