...
public class Dijkastra_path {
	...
	static class Graph {
		...
		public void Dijskastra_path(int srcVal) {
			int V = alist.size();
			...
			for (int i = 0; i < V; i++) {
				...
				System.out.print("To node " + nodeVal + " -> ");
				if(distance[i] == Integer.MAX_VALUE) {
					System.out.println("∞");
				}else {
					System.out.print("Cost = " + distance[i] + ", Path = ");
					printPath(parent, i);
					System.out.println();
				}
			}
		...
		private void printPath(int[] parent, int idx) {
			if(idx == -1) return;
			printPath(parent,parent[idx]);
			System.out.print(alist.get(idx).get(0).data + " ");
		}
	}
